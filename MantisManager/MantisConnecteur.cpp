...
namespace MantisManager
{
	...
	void MantisConnecteur::recupererTicketsDuProjet(QVector<MantisData::Ticket>&listeTicket, const QString nomDuProjet, const QString& user, const QString& password) const
	{
		MantisConnect mantisConnect;
		...
		foreach(ticket, list)
		{
			MantisData::Ticket ticketMantis(ticket.id(), ticket.summary(), ticket.status().id(), ticket.severity().id());
	...
	void MantisConnecteur::recupererTicketDeLaVersionsDuProjet(QVector<MantisData::Ticket>&listeTickets, const QString nomDuProjet, const QString nomVersion, const QString& user, const QString& password) const
	{
		MantisConnect mantisConnect;
		qint64 idProjet = mantisConnect.mc_project_get_id_from_name(user, password, nomDuProjet);
		...
		foreach(filter, list)
		{
			if (filter.name() == nomVersion)
			{
				...
				{
					MantisData::Ticket ticketMantis(ticket.id(), ticket.summary(), ticket.status(), ticket.severity());
	...
	void MantisConnecteur::livrerVersion(const QString& version, const QString& projet, const QString& user, const QString& password) const
	{
		MantisConnect mantisConnect;
		qint64 version_id;
		qint64 projet_id = mantisConnect.mc_project_get_id_from_name(user, password, projet);
	...
	void MantisConnecteur::recupererStatut(QVector<MantisData::Status>&listeStatuts, const QString& user, const QString& password) const
	{
		MantisConnect mantisConnect;
